...
def main():
    ...
    p.add_argument("--decomp", nargs="+", default=["comd"],
                   choices=["comd", "vmd", "svmd", "none"])
        ...
        for ax in args.axes:
            ...
            for snr in args.snr:
                ...
                for dec in args.decomp:
                    # "none" is the no-decomposition baseline: a single raw-signal
                    # channel, so K=1 is the only meaningful value regardless of --K.
                    for K in ([1] if dec == "none" else args.K):
